...
Todos os algoritmos são implementados do zero — sem bibliotecas de grafo externas. Na API pública, `start` e os nós retornados continuam sendo **nomes** (`str`); internamente, `start` é resolvido para um índice, os vizinhos são obtidos varrendo a linha correspondente de `graph.matrix` por entradas `!= 0.0`, e os índices são traduzidos de volta para nomes via `graph.nodes[i]` antes de retornar. `bfs` usa `Queue` (`src/types/queue.py` — fila própria, lista ligada, O(1) enqueue/dequeue, no lugar de `collections.deque`); `dfs` usa pilha explícita (sem recursão), marcando visitado no `pop` (não no `push`) para reproduzir a ordem de uma DFS recursiva.
...
| 16/06/2026 | 2.3 | `traversal.py` implementado por completo (`bfs`, `dfs`, `reachable`, `connected_components`, `count_components`, `is_connected`, `minimum_spanning_tree`), usando `Queue` própria (`src/types/queue.py`) em vez de `collections.deque` | Lucas Antunes |
